...
async function loadActionLog(handID, actionSeq) {
    const result = await client_1.ddb.send(new client_dynamodb_1.GetItemCommand({
        ...
        Key: {
            ...
            [fields_1.FIELDS.ACTION_LOG.ACTION_SEQ]: { N: actionSeq.toString() },
        },
    }));
    if (!result.Item) {
        return null;
...
async function writeAction(entry) {
    await client_1.ddb.send(new client_dynamodb_1.PutItemCommand({
        ...
        ConditionExpression: `attribute_not_exists(${fields_1.FIELDS.ACTION_LOG.ACTION_SEQ})`,
    }));
}
