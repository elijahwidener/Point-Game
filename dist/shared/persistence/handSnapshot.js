...
async function loadHandSnapshot(tableId, handSeq) {
    const result = await client_1.ddb.send(new client_dynamodb_1.GetItemCommand({
        ...
        Key: {
            [fields_1.FIELDS.HAND_SNAPSHOTS.TABLE_ID]: { S: tableId },
            [fields_1.FIELDS.HAND_SNAPSHOTS.HAND_SEQ]: { N: handSeq.toString() },
        },
    }));
...
async function writeHandSnapshot(tableID, handSeq, gameState) {
    ...
    await client_1.ddb.send(new client_dynamodb_1.PutItemCommand({
        ...
        ConditionExpression: `attribute_not_exists(${fields_1.FIELDS.HAND_SNAPSHOTS.TABLE_ID}) AND attribute_not_exists(${fields_1.FIELDS.HAND_SNAPSHOTS.HAND_SEQ})`
    }));
}
